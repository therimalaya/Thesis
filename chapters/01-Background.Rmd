...
With a transformation defined as $\mathbf{z} = \mathbf{Rx}$ and $\mathbf{w} = \mathbf{Qy}$ with $\mathbf{R}_{p\times p}$ and $\mathbf{Q}_{m\times m}$ random orthogonal rotation matrices, model \@ref(eq:linear-model) can be rewritten as,
...
```{r relcomp-example, fig.cap="Relevant components at two different set of positions and two different levels of multicollinearity. The points represents the correlation of predictor components and the response variable.", fig.scap="Relevant Components and Multicollinearity", fig.asp=0.5}
strp_lbl <- c(
...
The concept of relevant components also can be extended to the response such that a subspace contains the information relevant for a model. The concept is implemented in the simultaneous envelope [@cook2015simultaneous] and the response envelope [@cook2010envelope] methods.
